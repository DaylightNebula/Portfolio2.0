 ...
 * `SchemaElement.Array(subElementKey: SchemaElement, size: Int)` - An array of the given size must be at this location where the elements of the aray must match the `SchemaElement` given as `subElementKey`.
...
dependencies {
  ...
  // 0.4.4 may need to be changed to the lastest version in case I forgot to update this readme file.
