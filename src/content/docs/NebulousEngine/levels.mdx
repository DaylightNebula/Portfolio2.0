...
## The Levels Plugin
The `Levels` plugin is a Bevy plugin that handles your levels.  The plugin takes the enum created above and initializes an instance of the `Level` resource to assist in managing the current level, as well as, handles despawning any level-specific entities on a level change.
...
When a level changes, any entity marked with the `CurrentLevel` component will be despawned.
